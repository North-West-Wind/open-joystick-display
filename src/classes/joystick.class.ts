...
import NetworkDriver from "./drivers/network.driver.js";
import Driver from "./driver.class.js";
import ChromiumDriver from "./drivers/chromium.driver.js";
import RetroSpyDriver from "./drivers/retrospy.driver.js";
import { DataMappingButton } from "./data.js";
//import HIDDriver from "./drivers/hid.driver.js";

export default class Joystick {
	config: Config;
	profiles: Profiles;
	driver: string;
	lastButton: string | number;
	dpadKeywords: string[];
	cpadKeywords: string[];
	...
	checkRaw() {
		...
		for (let i = 0; i < directionalCount; i++) {
			...
			let offset = this.checkAnalog(axis1, axis2, 0);
	...
	checkArcadeStick(buttonMapping: DataMappingButton[]) {
		...
		const buttons: { [key: string]: undefined | number } = {
			'UP': undefined,
			'LEFT': undefined,
			'RIGHT': undefined,
			'DOWN': undefined
	...
	checkFixedTrigger(axisIndex: number, val: string) {
		const joystick = this.getCurrentDriver().getJoystick()!;
		const axis = joystick.axes[axisIndex];
